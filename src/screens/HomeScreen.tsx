...
export function HomeScreen({ onOpenTool, onBrowse, onPremium }: { onOpenTool: (tool: ClinicalTool) => void; onBrowse: () => void; onPremium: () => void }) {
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <View style={styles.header}><View><Text style={styles.eyebrow}>MEDICAL TOOLBOX</Text><Text style={styles.greeting}>Bonjour 👋</Text></View><Pressable onPress={onPremium} style={styles.premiumChip}><Ionicons name="diamond" size={14} color="#8A5907" /><Text style={styles.premiumChipText}>PREMIUM</Text></Pressable></View>
...
const styles = StyleSheet.create({
  screen: { flex: 1, backgroundColor: colors.canvas }, content: { padding: spacing.lg, paddingBottom: 38 }, header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, eyebrow: { color: colors.teal, fontSize: 10, fontWeight: '900', letterSpacing: 1.7 }, greeting: { marginTop: 4, color: colors.muted, fontSize: 14, fontWeight: '600' }, premiumChip: { height: 37, paddingHorizontal: 11, borderRadius: radius.pill, backgroundColor: '#FFF0C7', flexDirection: 'row', alignItems: 'center', gap: 6 }, premiumChipText: { color: '#8A5907', fontSize: 9, fontWeight: '900', letterSpacing: .6 },
  ...
  premiumBanner: { marginTop: 14, padding: 15, borderRadius: radius.md, borderWidth: 1, borderColor: '#E8D59D', backgroundColor: '#FFFAEC', flexDirection: 'row', alignItems: 'center', gap: 11 }, premiumIcon: { width: 39, height: 39, borderRadius: 13, backgroundColor: '#FFF0C7', alignItems: 'center', justifyContent: 'center' }, premiumCopy: { flex: 1 }, premiumTitle: { color: colors.ink, fontSize: 13, fontWeight: '900' }, premiumText: { marginTop: 3, color: colors.muted, fontSize: 10 },
});
